...
import gr1mly4memes.papercut.PapercutLogger;
import gr1mly4memes.papercut.config.ConfigConvert;
import gr1mly4memes.papercut.config.ConfigVerify;
import gr1mly4memes.papercut.config.GlobalConfig;
import gr1mly4memes.papercut.config.RemovedConfig;
...
public class GlobalConfigManager {
            ...
            if (Modifier.isStatic(field.getModifiers())) {
                ...
                if (globalConfig != null) {
                    try {
                        ...
                        PapercutConfig.config.addDefault(verifiedConfig.path, defValue);

                        try {
                            Object savedValue = PapercutConfig.config.get(verifiedConfig.path);
                            if (isEnumConfig) {
                            ...
                            PapercutConfig.config.set(verifiedConfig.path, defValue);
                            PapercutLogger.LOGGER.warning(e.getMessage() + ", reset to " + defValue);
                        }
    ...
    public record RemovedVerifiedConfig(RemovedConfig config, ConfigConvert<? super Object> convert, Field field,
        ...
        public void run() {
            if (config.transform()) {
                if (PapercutConfig.config.contains(path)) {
                    String string = PapercutConfig.config.get(path).toString();
                    try {
            ...
            PapercutConfig.config.set(path, null);
        }
    ...
    public record VerifiedConfig(GlobalConfig config, ConfigVerify<? super Object> verify, Field field, String path) {
        ...
        public void set(String realValue) throws IllegalArgumentException {
            ...
            try {
                ...
                PapercutConfig.config.set(path, verify instanceof ConfigVerify.EnumConfigVerify ? realValue.toUpperCase(Locale.ROOT) : value);
                PapercutConfig.save();
        ...
        public Object get() {
            try {
                ...
                PapercutLogger.LOGGER.log(Level.SEVERE, "Failure to get " + path + " value", e);
                return "<VALUE ERROR>";
            }
        }
